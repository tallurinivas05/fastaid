...
function currentBooking(){if(bookingId){const own=state.bookings.find(b=>b.id===bookingId);if(own)return own;}return state.bookings.find(b=>b.status!=='COMPLETED')||null}
...
async function bookNow(){const btn=$('#bookBtn');btn.disabled=true;btn.textContent='SENDING REQUEST…';try{const b=await api('/api/bookings',{method:'POST',body:'{}'});bookingId=b.id;renderUser();toast('Request sent to nearest captain')}catch(e){btn.disabled=false;btn.textContent='BOOK NOW';toast(e.message,true)}}
...
function renderHospital(){
  ...
  // In this demo the same hospital dashboard is used to send a blood request and
  // to demonstrate the provider acceptance. Therefore only real requests created
  // by the user are shown here — never seeded/fake blood requests.
  const incoming=requests.filter(r=>r.status==='REQUEST_SENT');
  const acceptedIncoming=requests.filter(r=>r.status==='ACCEPTED');
  ...
  <div class="hospital-interface ${hospitalTab==='accepts'?'':'hidden'}" data-hospital-panel="accepts">
    <div class="card blood-dashboard blood-accept-interface">
      ...
      <p class="sub">Blood requests sent from this dashboard appear here for acceptance. Accepting one immediately updates the same request in SENT REQUESTS.</p>
